...
class ClientsController extends AppAdminController
{
    ...
    public function actionGenerateCsv(){
        $clints = Clients::find()->select('username')->asArray()->all();

        $CSV_str = '';
        foreach($clints as $client){
            $CSV_str .= $client['username'] . "\r\n";
        }

        $tmp = Yii::getAlias('@webroot/uploads/mail.csv');
        $CSV_str = iconv( "UTF-8", "cp1251",  $CSV_str );

        file_put_contents($tmp, $CSV_str);

        return Yii::$app->response->sendFile($tmp);

        //return $this->redirect("/clients/index");

//        return $this->render("generate-csv");
    }
}
